                    ...
                    <div>
                        <div class="table-scrollable table-scrollable-borderless">
                            <table class="table table-hover table-light">
                                ...
                                <tbody>
                                <template v-for="(item,index) in frequencyList">
                                    <tr>
    ...
    module.exports = {
        ...
        methods: {
            fetchData (pageNum, rowCount) {
                var me = this;
                this.$http.get('/api/frequency/list', {
                    params: {
                        rowCount: rowCount,
                        currentPage: pageNum,
            ...
            create(){
                router.push("/frequency/create");
            },
            remove(id){
                var me = this;
                confirm({
                    content: "是否删除当前监测频次信息？",
                    success: function () {
                        me.$http.get("/api/frequency/delete", {
                            params: {
                                id: id
                            }
                        }).then(response => {
            ...
            edit(item){
                var me = this;
                router.push("/frequency/change?id=" + item.id);
            },
            getData(){
                var me = this;
            ...
            removeAll(){
                ...
                if (me.selected.length == 0) {
                    error("至少需要选择一个监测频次信息");
                    return;
                }
                confirm({
                    content: "是否删除当前选中监测频次信息？",
                    success: function () {
                        me.$http.get("/api/frequency/deleteAll", {
                            params: {
